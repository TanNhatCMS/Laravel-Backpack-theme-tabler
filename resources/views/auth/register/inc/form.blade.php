<h2 class="card-title text-center my-4">{{ trans('tannhatcms::base.register') }}</h2>
<form role="form" method="POST" action="{{ route('backpack.auth.register') }}">
    @csrf
    <div class="mb-3">
        <label class="form-label" for="name">{{ trans('tannhatcms::base.name') }}</label>
    ...
    <div class="mb-3">
        <label class="form-label" for="{{ backpack_authentication_column() }}">{{ trans('tannhatcms::base.'.strtolower(config('backpack.base.authentication_column_name'))) }}</label>
    ...
    <div class="form-group">
        <div>
            <button tabindex="5" type="submit" class="btn btn-primary w-100">
                {{ trans('tannhatcms::base.register') }}
            </button>
        </div>
    </div>
</form>